...
import { Video, Calendar, Clock, RefreshCw, PlayCircle, Settings, ArrowLeft } from 'lucide-react';
...
export default function FacilitatorSessionPage() {
    ...
    // If already live and device check is done, show the room
            ...
            <div className="w-full">
                ...
                {!isLive ? (
                        ...
                        {isCompleted || isCancelled ? (
                            <div className="w-full bg-white shadow-lg border border-primary/10 overflow-hidden rounded-tl-2xl rounded-br-2xl rounded-tr-none rounded-bl-none p-8 text-center">
                                ...
                                <p className="text-primary/60 mb-6 text-lg">
                                    {isCompleted ? 'Attendance records and chat history are available for review.' : 'This session has been cancelled.'}
                                </p>
                                <Button onClick={() => router.back()} variant="outline" className="rounded-tl-md rounded-br-md rounded-tr-none rounded-bl-none font-bold">
                                    Back to Classroom
                                </Button>
                            </div>
                        ) : (
                            <div className="w-full bg-white shadow-lg border border-primary/10 overflow-hidden rounded-tl-2xl rounded-br-2xl rounded-tr-none rounded-bl-none p-8">
                                <div className="text-center mb-8">
                                    ...
                                    <p className="text-primary/60 text-lg max-w-2xl mx-auto">
                                        ...
                                        You will be able to share your screen and webcam.
                                    </p>
                                </div>

                                <div className="bg-primary/5 p-6 rounded-tl-xl rounded-br-xl rounded-tr-none rounded-bl-none border border-primary/10 mb-8 max-w-3xl mx-auto">
                                    <h3 className="font-bold text-primary mb-4 text-lg">Before you start:</h3>
                                    <ul className="space-y-3 text-sm text-primary/70">
                                        <li className="flex items-start gap-3">
                                            <span className="text-accent text-xl">•</span>
                                            <span>Ensure you have adequate lighting and a quiet environment</span>
                                        </li>
                                        <li className="flex items-start gap-3">
                                            <span className="text-accent text-xl">•</span>
                                        ...
                                        <li className="flex items-start gap-3">
                                            <span className="text-accent text-xl">•</span>
                                            <span>Have your slides or materials ready for screen sharing</span>
                                        </li>
                                    </ul>
                                </div>
                                ...
                                <div className="max-w-md mx-auto">
                                    <Button
                                        ...
                                        disabled={isStarting}
                                        className="w-full bg-accent hover:bg-accent/90 text-white h-14 text-lg rounded-tl-xl rounded-br-xl rounded-tr-none rounded-bl-none font-bold shadow-lg"
                                    >
                                        {isStarting ? (
                                            <>
                    ...
                    deviceCheckComplete === false ? (
                        ...
                        <LiveRoom session={session} isInstructor={true} />
                    )
                )}
            </div>
        </div>
