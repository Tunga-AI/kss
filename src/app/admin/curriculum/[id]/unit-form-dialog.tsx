...
export function UnitFormDialog({ courseId, unit, onClose }: UnitFormDialogProps) {
    ...
    const initialDate = unit?.scheduledStartDate ? unit.scheduledStartDate.toDate().toISOString().split('T')[0] : '';
    const initialStartTime = unit?.scheduledStartDate ? unit.scheduledStartDate.toDate().toTimeString().slice(0, 5) : '';
    const initialEndTime = unit?.scheduledEndDate ? unit.scheduledEndDate.toDate().toTimeString().slice(0, 5) : '';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            let scheduledStartDate: Timestamp | undefined = undefined;
            let scheduledEndDate: Timestamp | undefined = undefined;

            if (scheduleData.date && scheduleData.startTime) {
                const start = new Date(`${scheduleData.date}T${scheduleData.startTime}`);
                scheduledStartDate = Timestamp.fromDate(start);

                if (scheduleData.endTime) {
                    const end = new Date(`${scheduleData.date}T${scheduleData.endTime}`);
                    scheduledEndDate = Timestamp.fromDate(end);
                }
            }
            ...
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-6 mt-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="space-y-2">
                            <Label htmlFor="date" className="text-primary font-bold text-xs uppercase tracking-widest">
                                Date
                            </Label>
                            <Input
                                id="date"
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="startTime" className="text-primary font-bold text-xs uppercase tracking-widest">
                                Start Time
                            </Label>
                            <Input
                                id="startTime"
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="endTime" className="text-primary font-bold text-xs uppercase tracking-widest">
                                End Time
                            </Label>
                            <Input
                                id="endTime"
