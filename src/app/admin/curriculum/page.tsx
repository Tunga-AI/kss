...
import { Plus, RefreshCw, BookOpen, Edit, Eye, Trash2, PlayCircle, Building, Users } from "lucide-react";
...
export default function AdminCurriculumPage() {
    ...
    const handleDelete = async (courseId: string) => {
        if (!firestore || !confirm('Are you sure you want to delete this course? This will also delete all units within it.')) return;

        try {
            setDeleting(courseId);
            await deleteLearningCourse(firestore, courseId);
        } catch (error) {
            ...
            <div className="w-full">
                ...
                <Tabs defaultValue="public" className="w-full">
                    ...
                    <TabsContent value="public" className="m-0 p-0 border-none">
                        <CourseTable courses={publicCourses} handleDelete={handleDelete} deleting={deleting} getStatusColor={getStatusColor} emptyMessage="No public courses found" />
                    </TabsContent>

                    <TabsContent value="corporate" className="m-0 p-0 border-none">
                        <CourseTable courses={corporateCourses} handleDelete={handleDelete} deleting={deleting} getStatusColor={getStatusColor} emptyMessage="No corporate courses found" />
                    </TabsContent>
                </Tabs>
            </div>
        </div>
...
function CourseTable({ courses, handleDelete, deleting, getStatusColor, emptyMessage }: any) {
    return (
                            ...
                            <TableRow key={course.id} className="hover:bg-primary/5 transition-colors group border-primary/10">
                                ...
                                <TableCell className="px-6 py-4 text-right">
                                    <div className="flex justify-end gap-2  transition-opacity">
                                        ...
                                        >
                                            <Link href={`/a/curriculum/${course.id}`} className="flex items-center gap-2">
                                        ...
                                        <Button
                                            variant="ghost"
                                            size="sm"
                                            className="h-8 shadow-none hover:bg-red-50 rounded-tl-md rounded-br-md rounded-tr-none rounded-bl-none"
                            ...
                            <TableRow>
                                <TableCell colSpan={5} className="text-center py-20">
                                    <div className="flex flex-col items-center gap-2">
                                        ...
                                        <Button
                                            asChild
                                            className="mt-4 bg-accent hover:bg-accent/90 text-white rounded-tl-lg rounded-br-lg rounded-tr-none rounded-bl-none"
                                        >
                                            <Link href="/a/curriculum/new">
                                                <Plus className="h-4 w-4 mr-2" />
                                                Create First Course
