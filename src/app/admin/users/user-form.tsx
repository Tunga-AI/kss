...
import { useUsersFirestore, useStorage } from '@/firebase';
...
export function UserForm({ user }: { user?: Partial<User> }) {
    ...
    let title = isNew ? 'Add Member' : 'Edit Member';
    let description = isNew
        ? 'Initialize a new identity within the system registry.'
        : `Modify access parameters for ${user?.name}.`;

    if (isNew) {
        if (formData.role === 'Learner') {
            title = 'Enroll Learner';
            description = "This creates a profile in both the main Users list and the Learners section.";
        } else if (formData.role === 'Facilitator') {
            title = 'Onboard Facilitator';
            description = "This creates a profile for a new Facilitator identity.";
        }
    }
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                ...
                                <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">
                                    {title} <span className="text-white/40 font-mono text-xl ml-2">{isNew ? '' : `#${user?.id}`}</span>
                                </h1>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            >
                                {isSaving ? (
                                    ...
                                    <><Save className="h-5 w-5 mr-2" /> Commit Records</>
                                )}
                            </Button>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        <Card className="bg-primary text-white border border-primary/10 rounded-tl-3xl rounded-br-3xl shadow-xl p-8 relative overflow-hidden">
                            ...
                            <div className="space-y-8 relative z-10">
                                <div className="space-y-4">
                                    <Label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-1">Authorization Role</Label>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <Select value={formData.status} onValueChange={(value) => handleSelectChange('status', value)} required>
                                        ...
                                        <SelectContent className="rounded-xl border-primary/10">
                                            <SelectItem value="Active" className="font-bold">Active Directory</SelectItem>
