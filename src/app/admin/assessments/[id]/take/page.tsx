...
import { ArrowLeft, RefreshCw, CheckCircle, XCircle, FileText, Calendar, Clock, TrendingUp, AlertCircle, Loader2 } from 'lucide-react';
...
export default function AdminTakeAssessmentPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    ...
    const handleStartAssessment = async () => {
        ...
        try {
            const newAttemptId = await startAssessment(firestore, {
                ...
                admissionId: 'admin-test-' + Date.now(),
                programId: assessment.programId || 'admin-test',
                programTitle: assessment.programTitle || 'Admin Review',
            });
        ...
        return (
            <div className="w-full min-h-screen bg-gray-50/50 p-2 md:p-4 font-body">
                <div className="w-full max-w-3xl mx-auto">
                    <Link href="/a/assessments" className="inline-flex items-center text-primary hover:text-primary/80 font-medium mb-4">
            ...
            <div className="w-full min-h-screen bg-gray-50/50 p-2 md:p-4 font-body">
                <div className="w-full max-w-3xl mx-auto">
                    ...
                    <div className="bg-white border border-primary/10 rounded-tl-3xl rounded-br-3xl shadow-2xl overflow-hidden">
                        <div className={cn('p-8 md:p-12 text-white relative overflow-hidden', passed ? 'bg-gradient-to-br from-green-500 to-green-600' : 'bg-gradient-to-br from-primary to-primary/80')}>
                            <div className="absolute top-0 right-0 -mt-20 -mr-20 w-80 h-80 bg-white/10 rounded-full blur-3xl" />
                            <div className="relative z-10 text-center">
                                <div className="h-24 w-24 rounded-full mx-auto mb-6 flex items-center justify-center bg-white/20 backdrop-blur-sm">
                                    {passed ? <CheckCircle className="h-16 w-16 text-white" /> : <TrendingUp className="h-16 w-16 text-white" />}
                                </div>
                                <h1 className="text-3xl md:text-4xl font-bold mb-3">{passed ? 'Excellent Work!' : 'Assessment Complete!'}</h1>
                                <p className="text-white/90 text-lg mb-6">{passed ? 'You have successfully completed this test' : 'Your test session has been submitted'}</p>
                                <div className="inline-block bg-white/20 backdrop-blur-sm rounded-tl-2xl rounded-br-2xl px-8 py-4">
                                    <p className="text-white/70 text-sm font-bold uppercase tracking-wider mb-1">Your Score</p>
                                    <p className="text-5xl md:text-6xl font-black text-white">{score.toFixed(0)}%</p>
                                </div>
                            </div>
                        </div>
                        <div className="p-8 md:p-12 flex flex-col gap-4">
                            <Button onClick={() => router.push('/a/assessments')} className="w-full bg-accent hover:bg-accent/90 text-white rounded-tl-xl rounded-br-xl font-bold h-12 shadow-lg">Back to Management</Button>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            <div className="flex-1">
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <div>
                                        <h1 className="text-xl md:text-2xl font-bold text-primary">{assessment.title} (Admin Test Mode)</h1>
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                        <div>
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <Badge className="bg-accent text-white border-none font-black text-[9px] uppercase tracking-widest ml-2 hidden sm:inline-flex">Admin Test Mode</Badge>
                            </div>
                            <p className="text-white/80 text-lg font-medium">{assessment?.description || "Take this assessment to test the applicant experience"}</p>
                        </div>
