...
import {
    ArrowLeft, RefreshCw, FileText, Award, Calendar,
    AlertCircle, Sparkles, Printer, BarChart2, Target,
    Lightbulb, ChevronDown, ChevronUp, XCircle,
} from 'lucide-react';
...
function scoreToLabel(s10: number): string {
...
function scoreColorClass(label: string): string {
    switch (label) {
        case 'Very Low': return 'text-red-600';
        case 'Low': return 'text-orange-600';
        case 'Below Average': return 'text-yellow-600';
        case 'Average': return 'text-blue-600';
        case 'Above Average': return 'text-teal-600';
        case 'High': return 'text-green-600';
        case 'Very High': return 'text-emerald-700';
        default: return 'text-primary';
    }
}

function scoreBgClass(label: string): string {
    switch (label) {
        case 'Very Low': return 'bg-red-50 border-red-200';
        case 'Low': return 'bg-orange-50 border-orange-200';
        case 'Below Average': return 'bg-yellow-50 border-yellow-200';
        case 'Average': return 'bg-blue-50 border-blue-200';
        case 'Above Average': return 'bg-teal-50 border-teal-200';
        case 'High': return 'bg-green-50 border-green-200';
        case 'Very High': return 'bg-emerald-50 border-emerald-200';
    ...
    return (
        <div className="w-full">
            ...
            <div className="relative h-4 bg-primary/5 rounded-full overflow-hidden border border-primary/10">
                <div className={cn('h-full rounded-full transition-all duration-700', colorMap[label] ?? 'bg-primary')} style={{ width: `${pct}%` }} />
            </div>
            <div className="flex justify-between mt-2">
                {SCORE_LABELS.map((l, i) => (
                    <span key={i} className={cn('text-[9px] font-black uppercase tracking-widest px-1', l === label ? scoreColorClass(label) : 'text-primary/20')}>{l}</span>
                ))}
            </div>
        </div>
    );
...
function CompetencyRadar({ results }: { results: { name: string; score10: number }[] }) {
    ...
    const pts = results.map((res, i) => {
        ...
        return {
            dx: cx + rr * Math.cos(angle), dy: cy + rr * Math.sin(angle),
            fx: cx + r * Math.cos(angle), fy: cy + r * Math.sin(angle),
            lx: cx + (r + 22) * Math.cos(angle), ly: cy + (r + 22) * Math.sin(angle),
            label: res.name,
        };
    });
    return (
        <svg viewBox="0 0 300 300" className="w-full max-w-xs mx-auto">
            {[2, 4, 6, 8, 10].map(v => {
                const rr = (v / 10) * r;
                const polygon = results.map((_, i) => {
                    const angle = (2 * Math.PI * i) / n - Math.PI / 2;
                    return `${cx + rr * Math.cos(angle)},${cy + rr * Math.sin(angle)}`;
                }).join(' ');
                return <polygon key={v} points={polygon} fill="none" stroke="#e5e7eb" strokeWidth="1" />;
            })}
            ...
            {pts.map((p, i) => (
                <text key={i} x={p.lx} y={p.ly} textAnchor="middle" dominantBaseline="middle" fontSize="7.5" fill="#374151" fontWeight="600">
...
export default function AssessmentReportPage({ params }: { params: Promise<{ id: string; attemptId: string }> }) {
        ...
        const load = async () => {
            ...
            try {
                ...
                if (atSnap.exists()) {
                    ...
                    setAttempt(atData);
                    const asSnap = await getDoc(doc(usersFirestore, 'assessments', id));
        ...
        try {
            const input: AssessmentReportInput = {
                ...
                competencyResults: competencyResults.map(r => ({
                    ...
                    scorePercent: Math.round(r.scorePercent),
                    scoreOutOf10: Math.round(r.scoreOutOf10 * 10) / 10,
                })),
            };
    ...
    return (
        <div className="w-full min-h-[calc(100vh-6rem)] bg-gray-50/50 p-4 md:p-8 font-body print:bg-white">
            <div className="w-full">

                {/* ── Back + Print row ──────────────────────────────────────── */}
                <div className="flex items-center justify-between gap-4 mb-8 flex-wrap print:hidden">
                    <Link
                        href={`/a/assessments/${id}/attempts/${attemptId}`}
                        className="inline-flex items-center text-primary hover:text-primary/80 font-medium bg-white px-4 py-2 rounded-tl-lg rounded-br-lg shadow-sm border border-primary/10 transition-all hover:bg-primary hover:text-white group"
                    >
                        <ArrowLeft className="h-4 w-4 mr-2 transition-transform group-hover:-translate-x-1" />
                        Back to Review
                    </Link>
                    <div className="flex items-center gap-3">
                        {competencyResults.length === 0 && !report && (
                            <p className="text-xs text-yellow-700 bg-yellow-50 border border-yellow-200 px-3 py-1.5 rounded-lg font-medium hidden sm:block">
                                ⚠️ Tag questions with competencies to enable AI analysis
                            </p>
                        )}
                        <Button
                        ...
                        >
                            {generating
                                ...
                                : <><Sparkles className="h-4 w-4 mr-2" />{report ? 'Regenerate AI Report' : 'Generate AI Report'}</>
                            }
                        </Button>
                ...
                {/* ── Hero / Title Card ─────────────────────────────────────── */}
                <Card className="rounded-tl-3xl rounded-br-3xl rounded-tr-none rounded-bl-none bg-primary text-white overflow-hidden mb-8 shadow-2xl relative border-none print:rounded-none print:shadow-none">
                        ...
                        <div className="flex flex-col md:flex-row items-start justify-between gap-6">
                            <div className="flex-1">
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <span className="text-[10px] font-black uppercase tracking-widest text-white/40">Competency Report</span>
                                </div>
                                <h1 className="text-3xl md:text-4xl font-black tracking-tight mb-1">{attempt.userName}</h1>
                                <p className="text-white/60 text-sm mb-4">{attempt.userEmail}</p>
                                <p className="text-xl font-bold text-white/90">{assessment.title}</p>
                                {assessment.programTitle && <p className="text-white/50 text-sm">{assessment.programTitle}</p>}
                ...
                <div className="space-y-8">
                    ...
                    <section>
                        <div className="flex items-center gap-3 mb-4">
                            <div className="h-8 w-8 rounded-tl-lg rounded-br-lg bg-accent/10 flex items-center justify-center text-accent font-black text-sm shrink-0">1</div>
                            <h2 className="text-xl font-black text-primary uppercase tracking-wide">The General Description</h2>
                        </div>
                        <Card className="p-6 md:p-8 rounded-tl-2xl rounded-br-2xl rounded-tr-none rounded-bl-none border border-primary/10 shadow-md">
                            {report?.assessmentDescription ? (
                                <div className="space-y-3">
                                    {report.assessmentDescription.split('\n').filter(Boolean).map((para, i) => (
                                        <p key={i} className="text-sm text-primary/80 leading-relaxed">{para}</p>
                                    ))}
                                </div>
                            ) : (
                                <>
                                    ...
                                    {competencyResults.length > 0 && (
                                        <div className="mt-4 p-4 bg-primary/5 rounded-xl">
                                            <p className="text-[10px] font-black text-primary/40 uppercase tracking-widest mb-3">Competencies Measured</p>
                                    ...
                                    {!report && (
                                        <div className="mt-6 p-4 bg-accent/5 border border-accent/20 rounded-xl flex items-start gap-3">
                                            ...
                                            <p className="text-sm text-primary/70">Click <strong>"Generate AI Report"</strong> above to receive a personalised professional narrative for each competency, including coaching tips.</p>
                                        </div>
                                    )}
                                </>
                            )}
                    ...
                    <section>
                        <div className="flex items-center gap-3 mb-4">
                            <div className="h-8 w-8 rounded-tl-lg rounded-br-lg bg-accent/10 flex items-center justify-center text-accent font-black text-sm shrink-0">2</div>
                            <h2 className="text-xl font-black text-primary uppercase tracking-wide">Overall Score</h2>
                        </div>
                        <Card className="p-6 md:p-8 rounded-tl-2xl rounded-br-2xl rounded-tr-none rounded-bl-none border border-primary/10 shadow-md">
                            <div className="flex flex-col md:flex-row gap-8 items-start">
                                <div className={cn('shrink-0 rounded-2xl border-2 p-6 text-center min-w-[160px]', scoreBgClass(overallLabel))}>
                                    <p className="text-[10px] font-black uppercase tracking-widest opacity-60 mb-1">Score</p>
                                    <p className={cn('text-5xl font-black tracking-tighter', scoreColorClass(overallLabel))}>{overallScore10.toFixed(1)}</p>
                                    <p className="text-xs font-bold text-primary/40 mt-1">out of 10</p>
                                    <div className={cn('mt-2 text-xs font-black uppercase tracking-widest', scoreColorClass(overallLabel))}>{overallLabel}</div>
                                </div>
                                    ...
                                    {report?.overallNarrative && (
                                        <div className="mt-6 space-y-3">
                                            {report.overallNarrative.split('\n').filter(Boolean).map((p, i) => (
                                                <p key={i} className="text-sm text-primary/80 leading-relaxed">{p}</p>
                                            ))}
                                        </div>
                                    )}
                                </div>
                    ...
                    {competencyResults.length > 0 && (
                        <section>
                            <div className="flex items-center gap-3 mb-4">
                                <div className="h-8 w-8 rounded-tl-lg rounded-br-lg bg-accent/10 flex items-center justify-center text-accent font-black text-sm shrink-0">3</div>
                                <h2 className="text-xl font-black text-primary uppercase tracking-wide">The Graph — Competency Breakdown</h2>
                            </div>
                            <Card className="p-6 md:p-8 rounded-tl-2xl rounded-br-2xl rounded-tr-none rounded-bl-none border border-primary/10 shadow-md">
                                            ...
                                            return (
                                                <div key={r.competencyId}>
                                                    <div className="flex justify-between items-center mb-1.5">
                                                        ...
                                                        <span className={cn('text-xs font-black', scoreColorClass(lbl))}>
                                                            {r.scoreOutOf10.toFixed(1)}/10 · {lbl}
                                                        </span>
                                                    </div>
                                                    <div className="h-3 bg-primary/5 rounded-full overflow-hidden border border-primary/10">
                                                        <div className={cn('h-full rounded-full', colorBar[lbl] ?? 'bg-primary')} style={{ width: `${(r.scoreOutOf10 / 10) * 100}%` }} />
                                                    </div>
                    ...
                    <section>
                        <div className="flex items-center gap-3 mb-4">
                            <div className="h-8 w-8 rounded-tl-lg rounded-br-lg bg-accent/10 flex items-center justify-center text-accent font-black text-sm shrink-0">4</div>
                        ...
                        {generating && (
                            <Card className="p-12 rounded-tl-2xl rounded-br-2xl border border-accent/20 shadow-md flex flex-col items-center justify-center gap-4 text-center mb-6">
                                <div className="relative">
                                    <RefreshCw className="h-10 w-10 animate-spin text-accent" />
                                    <Sparkles className="h-4 w-4 text-accent absolute -top-1 -right-1 animate-pulse" />
                                </div>
                                <p className="font-black text-primary">AI is analysing competency results…</p>
                                <p className="text-sm text-primary/50">Generating personalised narratives and coaching tips</p>
                            </Card>
                        )}
                        ...
                        {competencyResults.length === 0 && !generating && (
                            <Card className="p-8 rounded-tl-2xl rounded-br-2xl border border-yellow-200 bg-yellow-50 text-center mb-6">
                                ...
                                <p className="font-bold text-yellow-800 mb-1">No Competency Tags Found</p>
                                <p className="text-sm text-yellow-700">Edit the assessment and assign a competency to each question to enable detailed competency analysis.</p>
                                <Link href={`/a/assessments/${id}/edit`} className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-accent hover:underline">Edit Assessment →</Link>
                            </Card>
                        )}
                        ...
                        <div className="space-y-6">
                            {competencyResults.map((r, idx) => {
                                ...
                                const badgeColors: Record<string, string> = {
                                    ...
                                    'Above Average': 'bg-teal-100 text-teal-700', 'High': 'bg-green-100 text-green-700',
                                    'Very High': 'bg-emerald-100 text-emerald-700',
                                };
                                return (
                                    <Card key={r.competencyId} className={cn('rounded-tl-2xl rounded-br-2xl rounded-tr-none rounded-bl-none border shadow-md overflow-hidden', scoreBgClass(lbl))}>
                                        <button
                                            ...
                                            className="w-full p-6 flex items-center gap-4 text-left hover:bg-black/5 transition-colors print:cursor-default"
                                        >
                                            ...
                                            <div className="flex items-center gap-3 shrink-0">
                                                <div className="text-center">
                                                    <p className={cn('text-3xl font-black tracking-tight', scoreColorClass(lbl))}>{r.scoreOutOf10.toFixed(1)}</p>
                                                    <p className="text-[10px] text-primary/40 font-bold">/ 10</p>
                                                </div>
                                                <Badge className={cn('text-[9px] font-black uppercase tracking-widest border-none', badgeColors[lbl] ?? 'bg-primary/10 text-primary')}>{lbl}</Badge>
                                                {isExpanded ? <ChevronUp className="h-4 w-4 text-primary/40 print:hidden" /> : <ChevronDown className="h-4 w-4 text-primary/40 print:hidden" />}
                                            </div>
                                        </button>

                                        <div className={cn('border-t border-primary/10 bg-white', isExpanded ? 'block' : 'hidden print:block')}>
                                                ...
                                                {narrative ? (
                                                    ...
                                                    <>
                                                        {r.categoryDescription && (
                                                            <div className="border-t border-primary/5 pt-4">
                                                                <p className="text-[10px] font-black text-primary/40 uppercase tracking-widest mb-2">About This Category</p>
                                                                <p className="text-sm text-primary/60 leading-relaxed">{r.categoryDescription}</p>
                                                            </div>
                                                        )}
                                                        <div className="flex items-start gap-3 p-4 bg-accent/5 border border-accent/20 rounded-xl">
                                                            <Sparkles className="h-5 w-5 text-accent shrink-0 mt-0.5" />
                                                            <p className="text-sm text-primary/60">Generate the AI report to get personalised commentary and development recommendations for this competency.</p>
                                                        </div>
                                                    </>
                                                )}
                                            </div>
                        ...
                        {report?.overallRecommendation && (
                            <Card className="mt-8 p-6 md:p-8 rounded-tl-2xl rounded-br-2xl rounded-tr-none rounded-bl-none border border-accent/20 bg-accent/5 shadow-md">
                                <div className="flex items-start gap-4">
                                    ...
                                    <div>
                                        <p className="text-[10px] font-black text-accent uppercase tracking-widest mb-2">Overall Recommendation</p>
                    ...
                    <div className="text-center text-[10px] text-primary/30 font-bold uppercase tracking-widest pt-6 border-t border-primary/5">
                        KSS © {new Date().getFullYear()} · Report generated {format(new Date(), 'PPP')} · Confidential
                    </div>
                </div>
            </div>
        </div>
