...
import { FolderKanban, RefreshCw, Search, ChevronDown, User as UserIcon, Calendar, Filter, Mail, ArrowRight, Trash2 } from "lucide-react";
...
export default function AdmissionsPage() {
  ...
  useEffect(() => {
    const fetchCohorts = async () => {
      if (!firestore || !allAdmissions) return;
      ...
      try {
        ...
        setAdmissionsWithCohorts(allAdmissions);
      }
    };

    fetchCohorts();
  }, [firestore, allAdmissions]);

  const loading = admissionsLoading || (allAdmissions && admissionsWithCohorts.length === 0);
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    if (!firestore || !confirm('Are you sure you want to delete this admission record?')) return;
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <FolderKanban className="h-8 w-8 bg-accent/20 p-1.5 rounded-lg" />
                <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">Admissions Pipeline</h1>
              </div>
              <p className="text-white/80 text-lg font-medium">Monitor and manage applicant progress and enrollment</p>
            </div>
                  ...
                  <TableRow key={admission.id} className="hover:bg-primary/5 cursor-pointer transition-colors group border-primary/10">
                    ...
                    <TableCell className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        >
                          <Link href={`/a/admissions/${admission.id}`}>
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-8 shadow-none hover:bg-accent/10 hover:text-accent rounded-tl-md rounded-br-md rounded-tr-none rounded-bl-none transition-opacity"
                          onClick={(e) => handleDelete(admission.id, e)}
                        >
