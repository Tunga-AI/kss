...
gcloud builds submit --tag gcr.io/gonogasport/sales \
  --timeout=20m \
  -- \
  --build-arg NEXT_PUBLIC_FIREBASE_PROJECT_ID=msommii \
  --build-arg NEXT_PUBLIC_FIREBASE_API_KEY=AIzaSyB0zPzaidQLDpNyrh7ReaiRyNhp8HOAPfc \
  --build-arg NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN=msommii.firebaseapp.com \
  --build-arg NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET=msommii.firebasestorage.app \
  --build-arg NEXT_PUBLIC_PAYSTACK_PUBLIC_KEY=pk_test_3531fac258f9e1e40a92e942e313e046fbf49866 \
  --build-arg PAYSTACK_SECRET_KEY=sk_test_caf341ef6d5594a1069c7f9c9465fb2124150e07 \
  --build-arg NEXT_PUBLIC_APP_URL=https://sales-uurxypxfoa-uc.a.run.app

# Deploy to Cloud Run with runtime environment variables
gcloud run deploy sales \
  --image gcr.io/gonogasport/sales:latest \
  --region us-central1 \
  --platform managed \
  --set-env-vars="PAYSTACK_SECRET_KEY=sk_test_caf341ef6d5594a1069c7f9c9465fb2124150e07,FIREBASE_PRIVATE_KEY=-----BEGIN PRIVATE KEY-----
...
-----END PRIVATE KEY-----,FIREBASE_CLIENT_EMAIL=firebase-adminsdk-fbsvc@msommii.iam.gserviceaccount.com,EMAIL_HOST=smtp.gmail.com,EMAIL_PORT=587,EMAIL_USER=hi@kss.or.ke,EMAIL_PASSWORD=yvva uynv aqry kdmi,EMAIL_FROM_NAME=KSS Academy,APP_URL=https://sales-uurxypxfoa-uc.a.run.app"
